...
def load_and_split_data(seed=42):
    ...
    train_texts, val_texts, train_labels, val_labels = train_test_split(
        train_val_texts, train_val_labels, test_size=0.15 / 0.85, random_state=seed, stratify=train_val_labels
    )
...
def tokenize_and_embed(sentence, fasttext_model, max_len=32, embed_dim=300):
...
def create_embedded_sequences(texts, fasttext_model, max_len=32):
    embedded_sequences = []
    for text in texts:
        emb_seq = tokenize_and_embed(text, fasttext_model, max_len)
        embedded_sequences.append(emb_seq)
    return np.array(embedded_sequences, dtype=np.float32)


class SentimentDataset(Dataset):
    def __init__(self, embedded_sequences, labels):
        self.embedded_sequences = torch.tensor(embedded_sequences, dtype=torch.float32)
    ...
    def __getitem__(self, idx):
        return self.embedded_sequences[idx], self.labels[idx]


class LSTMClassifier(nn.Module):
    def __init__(self, input_dim=300, hidden_dim=128, num_layers=2, num_classes=3, dropout=0.3, bidirectional=True):
...
def plot_training_curves(metrics, save_path='outputs/lstm_training_curves.png'):
...
def plot_confusion_matrix(y_true, y_pred, save_path='outputs/lstm_confusion_matrix.png'):
    ...
    ax.set(xticks=np.arange(cm.shape[1]),
           ...
           title='Confusion Matrix - LSTM Classifier',
           ylabel='True Label',
...
def main():
    ...
    print("Creating embedded sequences...")
    train_sequences = create_embedded_sequences(train_texts, fasttext_model, max_len=32)
    val_sequences = create_embedded_sequences(val_texts, fasttext_model, max_len=32)
    test_sequences = create_embedded_sequences(test_texts, fasttext_model, max_len=32)
    ...
    best_model_state, best_val_f1, metrics = train_model(
        model, train_loader, val_loader, criterion, optimizer, scheduler, device, num_epochs=60
    )
    
    print(f"\nBest Validation Macro F1: {best_val_f1:.4f}")
    
    print("Plotting training curves...")
    plot_training_curves(metrics, save_path='outputs/lstm_training_curves.png')
    
    print("Saving best model...")
    torch.save(best_model_state, "best_sentiment_lstm_model.pth")
    ...
    plot_confusion_matrix(test_labels_list, test_preds, save_path='outputs/lstm_confusion_matrix.png')
